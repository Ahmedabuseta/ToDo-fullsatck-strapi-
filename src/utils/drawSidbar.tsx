...
import { IList, ISidebar } from "../interface/interfaces";
...
function DrawSidebar({
  ...
  const render = list.map(
    ({ title, type, color, icon, link }: ISidebar, idx: number) => (
      ...
      <Link to={link} onClick={handleCloseBar} className="no-underline ">
        <li
          key={idx}
          onClick={() => handleActive(idx + baseIdx)}
          className={`
      ${
        ...
        >
          <Badge
            ...
            color="#555"
            size="small"
            className={`${openMenu ? " hidden" : " hidden md:flex lg:hidden"}`}
            offset={[7, 5]}
          >
            {typee === "list" ? (
              <div
            ...
            <div
              className={`${
                openMenu
                  ...
                  : "flex gap-2 md:hidden items-center  lg:flex"
              }`}
            >
              {typee === "list" ? (
                <div
                  style={{ backgroundColor: `${color}` }}
                  className={`${color} bg-[${color}] mx-auto w-10 h-10 flex justify-center items-center rounded-md `}
                ></div>
              ) : (
                <span className="font-bold :">{icon}</span>
              )}

              <span className="capitalize  ">{type ? type : title}</span>
            </div>
            <span
              className={`${
                active === idx + baseIdx
                  ? "dark:bg-black bg-white"
                  : "dark:bg-sky-500 bg-[#ebebeb]"
              } dark:group-hover:bg-black group-hover:bg-white p-1 ps-2 pe-2 font-medium  ${
                openMenu ? " md:flex " : "md:hidden"
              }  flex lg:flex italic rounded-md`}
            >
              4
            </span>
          </>
